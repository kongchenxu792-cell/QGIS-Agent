...
- **多灾种可插拔，验证范围如实标注**：地震 / 火灾 = 合成数据验证；滑坡 = 合成数据 + 重庆真实坡度（>30° 极陡风险区）双重验证；洪涝 = 成都 / 重庆真实河流数据（buffer 近似）+ 合成数据双重验证；通用覆盖链（避难所对行政区覆盖率）= 成都真实（37 / 1284 点）+ 重庆真实（39 / 1619 点）验证。合成灾种仅验证链路可行性，真实灾种验证真实地理数据接入。
...
用中国真实数据（成都：行政区 / 人口格网 19365；重庆：行政区 / 人口格网，均 EPSG:3857，半径 500m）跑通三链，全部 PASS 并触发预警。双城均采用双口径验证：

| 城市 | 口径 A（官方挂牌） | 口径 B（含潜在载体） | 覆盖率 | 盲区率 | 人口覆盖率 | 状态 |
|------|------|------|------|------|------|------|
| 成都 | 37 点官方 | 1284 点（37 官方 + 1247 OSM） | 0.144% / 4.12% | 99.86% / 95.88% | 1.41% / 27.27% | 成功 |
| 重庆 | 39 点官方 | 1619 点（39 官方 + 1580 OSM） | 0.019% / 0.806% | 99.98% / 99.19% | 0.08% / 13.18% | 成功 |

- 数值为示例口径（挂牌=官方名录，全量=含学校/公园等潜在载体）；低覆盖率因危险区范围大 + 官方点少，属数据口径正常，非引擎异常
- 独立复算覆盖面积 vs 引擎：成都两口径、重庆 4 链（2 通用 + 洪涝 + 滑坡）相对偏差均 **0.0000%**（容差 ±1%）→ PASS
- 一致性校验（gap_rate vs 100-coverage_rate）：成都两口径、重庆两口径偏差均 **0.0000%** → PASS
- 预警：成都口径 A 0.14%、口径 B 4.12%；重庆口径 A 0.02%、口径 B 0.81%，均低于预警阈值 50.00%，建议核查避难所布点
- 双口径意义：口径 A（挂牌 0.14% / 0.02%）展示可信下限（官方挂牌数据，置信度最高）；口径 B（含潜在载体 4.12% / 0.81%）展示容量评估视角（学校/公园/运动场等潜在避难场所，依据国家标准）
- 注册表新增 `chengdu` 条目，现有 4 灾种零改动；重庆场景经 `scripts/run_chongqing.py` 同链直跑（引擎 / Guards / 模板 / CRS 零改动）
- 跑通脚本：`scripts/run_chengdu_dual.py`（成都）、`scripts/run_chongqing.py`（重庆）；对照表：`output/成都双口径/双口径对照表.md`、`output/重庆跑通/重庆跑通结果表.md`；运行记录：`output/成都双口径/run_record.json`、`output/重庆跑通/run_record.json`；CEO 实测复现（真实 UI）：[ceo_ui_final_20260831_170141.png](output/成都双口径/CEO_UI复现/ceo_ui_final_20260831_170141.png)
...
多灾种真实数据验证范围如实标注：通用覆盖链 = 成都（37 / 1284 点）+ 重庆（39 / 1619 点）；洪涝 = 成都 + 重庆真实河流 buffer 近似；滑坡 = 重庆真实坡度风险区（山城特色）。

| 城市 | 灾种 | 危险区数据源 | 覆盖率 | 状态 |
|------|------|------|------|------|
| 成都 | 洪涝 | 河流 300m buffer 淹没区近似（5122 条） | 0.1465% | 成功 |
| 重庆 | 洪涝 | 河流 300m buffer 淹没区近似 | 0.0468% | 成功 |
| 重庆 | 滑坡 | 坡度 >30° 极陡风险区（1156 面，slope_class=3） | 0.0125% | 成功 |

- 洪涝淹没区均为河流 buffer 近似示意（非水文模型结果），仅用于覆盖分析演示
- 滑坡风险区由坡度分级矢量（SRTM 30m）中 slope_class=3（>30° 极陡）生成，风险面与陡坡区域对应，几何 100% valid
- 各链独立复算覆盖面积 vs 引擎：相对偏差均 **0.0000%**（容差 ±1%）→ PASS
- 结果与报告：`output/重庆跑通/`（结果表 / 洪涝报告 / 滑坡报告 / run_record）；脚本：`scripts/run_chongqing.py`
...
│   ├── run_chongqing.py        # 重庆多灾种跑通脚本（39 官方 / 1619 全量 + 洪涝 + 滑坡）
│   ├── chengdu_flood_gen.py    # 成都洪涝淹没区生成（河流 buffer 近似）
...
└── output/
    ...
    ├── 成都洪涝/               # 成都洪涝淹没区 + 结果 + run_record
    ├── 重庆跑通/               # 重庆多灾种结果表 + 洪涝/滑坡报告 + run_record
    ├── 片A补充_LLM全链验证/
    ├── 落地证明包/             # 落地证明包
    └── ...
```
...
- **中国真实数据验证（成都双口径 / 重庆多灾种）**：[output/成都双口径/双口径对照表.md](output/成都双口径/双口径对照表.md)、[output/重庆跑通/重庆跑通结果表.md](output/重庆跑通/重庆跑通结果表.md)
