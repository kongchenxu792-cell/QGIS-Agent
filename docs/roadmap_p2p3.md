...
- 候选池查证（2026-08，Solo 复核，已核实最新）：① GLM-4.5-Air=106B 稀疏 MoE，服务器级，毙；② **Gemma 4 E4B（2026-04 发布，Apache 2.0，Ollama v0.20.0+ 可用）**为当前 4B 档劲敌，Gemma 3 已过时，待 25×3 实测；③ Hunyuan-7B JSON/生态弱，毙；④ DeepSeek 推理型、Llama 英文系与任务不适配；⑤ GLM 开源稠密端侧仅 GLM-4-9B-Chat（9B，4-bit≈5.5GB，卡 8GB 空闲线无余量）；⑥ Qwen3.6（2026-04）仅 27B/35B-A3B MoE/Plus，**无 4B 稠密**，故 qwen3.5-4b 仍是 Qwen 系 4B 档现役。结论：8GB 档当前对决 = qwen3.5-4b vs Gemma 4 E4B，由 P1.7 实测定。教训：涉及模型/版本的任务下发前必须 web 核实最新并引用来源，禁止凭记忆写过时型号。
