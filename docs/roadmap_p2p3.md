...
- 候选池查证（2026-08，Solo 复核时发现两处未核实采信已修正）：① GLM-4.5-Air=106B 稀疏 MoE，服务器级，毙（有据）；② **Gemma 3 4B：结论未定**——Gemma 3 代中文获赞（实测"中文理解惊艳"），且官方主打 140+ 语言，旧"英文系"标签过时；但针对本任务（中文防灾指令→JSON）无现成权威数据，**不凭道听途说淘汰，保留为待实测候选**；③ Hunyuan-7B JSON/生态弱，毙；④ DeepSeek 推理型、Llama 英文系与任务不适配；⑤ GLM 开源稠密端侧仅 GLM-4-9B-Chat（9B，4-bit≈5.5GB，卡 8GB 空闲线上无余量），更小的 GLM-4-Flash/4.5-Flash 为 API-only——GLM 系在 8GB 卡无合格对手。结论：qwen3.5-4b 为当前默认最优，**但「无对手」尚缺 Gemma 3 4B 的实测证据**；要坐实需跑一次 25×3 头对头（qwen3.5-4b vs Gemma 3 4B）。教训：候选淘汰必须引用可核实数据，禁止采信未核实转述。
