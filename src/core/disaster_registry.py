...
- names          : 三语名称 {zh, ja, en}
- risk_zone_layer: 危险区图层文件名（数据位于 temp/multi_disaster/，与
                   scripts/gen_multi_disaster_data.py 输出一致）
- template       : 模板实例名（src/core/templates/{template}.json）
- triggers       : 三语触发词（供片B UI 下拉 / 指令映射扩展使用）
- description    : 一句话说明

扩展新灾种：新增一条注册 + 在 temp/multi_disaster/ 下放置对应危险区
GPKG 即可，引擎与模板零改动（灾种可插拔）。
"""
...
def get_risk_zone_path(disaster_id: str) -> str:
    """获取灾种危险区图层绝对路径。"""
    layer = get_risk_zone_layer(disaster_id)
    if not layer:
        return ""
    return os.path.join(_DISASTER_DATA_DIR, layer)
